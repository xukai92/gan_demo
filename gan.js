...
var sigma = 0.1;
function pdf1(t) {
  var p = 1.0 / (sigma*Math.sqrt(2*Math.PI)) * Math.exp(-(t-mu)*(t-mu)/(2*sigma*sigma)) * 50;
  return p;
}
function sample1() {
  return Math.randn()*sigma + mu;
}
...
function sample2() {
  if(Math.random() < 0.5) { return Math.randn()*sigma1 + mu1; } else { return Math.randn()*sigma2 + mu2; }
}
var pdf = pdf1;
var sample = sample1;
var curd = 1;
...
var Net = function(nh, wscale) {
  ...
  var param = {}
  var grad_param = {}
  param.w1 = []; param.b1 = []; param.w2 = []; param.b2 = 0;
  grad_param.w1 = []; grad_param.b1 = []; grad_param.w2 = []; grad_param.b2 = 0.5;
  for(var i=0;i<nh;i++) {
    var c = i/nh;
  ...
  this.hcache = []; for(var i=0;i<nh;i++) { this.hcache[i] = 0; }
}
Net.prototype = {
  forward: function(t) {
  ...
  update: function() {
    ...
    var p = this.param.w2; var g = this.grad_param.w2; for(var i=0;i<nh;i++) { p[i] += -lr * (g[i] + reg*p[i]); g[i] = 0; }
    this.param.b2 += -lr * this.grad_param.b2; this.grad_param.b2 = 0;
  }
}
...
function render() {
  ...
  while(t<=1) {
    ...
    svg.append('line').attr('x1', t*(orix1-orix0) + orix0).attr('y1', orih)
    .attr('x2', x*(orix1-orix0) + orix0).attr('y2', transh)
    .attr('stroke', '#F00')
    .attr('stroke-width', '1px');
...
function start() {
  render();
  setInterval(function(){ for(var i=0;i<20;i++) { step(); } render(); }, 20);
}

function reset() {
  gen = new Net(30, 0.5); // generator
  disc = new Net(30, 0.5); // discriminator
}
